...
xob.Gadget.init = function(){
    ...
    // Check if the user has selected anyone in the past for this account.
    if (xob.Gadget.client.github_username != ''){
        xob.Gadget.client.load_profile(xob.Gadget.client.github_username, xob.Gadget.render_profile);
    } else {
        // force user to select a github user first
        xob.Gadget.select_user();
    }
};

/**
 * Display a single github user's full profile.
 */
xob.Gadget.render_profile = function(user){
    
    var panel = $('#details');
    panel.empty();
    
    var title = $('<div/>').appendTo(panel);
    $('<img/>', {src: user.get_picture()}).appendTo(title);
    $('<h1/>', {text: user.username}).appendTo(title);
    
    var details = $('<ul/>').appendTo(panel);
    $('<li/>', {text: "Name: " + user.full_name}).appendTo(details);
    $('<li/>', {text: "Company: " + user.company}).appendTo(details);
    $('<li/>', {text: "Location: " + user.loc}).appendTo(details);
    
    var repos_panel = $('<div/>', {'class': 'repos_panel'}).appendTo(panel);
    xob.Gadget.client.load_repos(user.username, function(repos){
        
        for (var i=0; i < repos.length; i++) {
            var repo = repos[i],
                repo_container = $('<div/>');
            
            $('<a/>', {text: repo.repo_name, href: repo.url, target: '_blank'}).appendTo(repo_container);
            $('<p/>', {text: repo.desc}).appendTo(repo_container);
            repo_container.appendTo(repos_panel);
        };
    });
    
    xob.Gadget.show_view('details');
};
    ...
    if (users !== undefined){
        ...
        if (users.length > 0){
            for (var u_i=0; u_i < users.length; u_i++) {
                ...
                // Select user button
                $('<input/>', {type: 'submit', value: 'select'}).data('user', user).click(function(e){
                    var user = $(this).data('user');
                    xob.Gadget.client.save_user(user, xob.Gadget.render_profile);
                }).appendTo(user_list_item);
